...
describe('business component adoption contract', () => {
  ...
  it('keeps the project archive workspace aligned to the Figma 43:317 grid', () => {
    ...
    expect(archive).toContain('grid-template-columns: 270px minmax(0, 1fr)')
    expect(archive).toContain(':scroll="{ minWidth: 760 }"')
    ...
    expect(archive).toContain("slotName: 'fileName', minWidth: 274")
    ...
    expect(archive).toContain("slotName: 'uploadedAt', width: 122")
    expect(archive).toContain("slotName: 'action', width: 150")
    expect(archive).toContain(':columns="fileColumns"')

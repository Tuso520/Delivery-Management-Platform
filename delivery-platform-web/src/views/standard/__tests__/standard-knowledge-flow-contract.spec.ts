...
describe('standard and knowledge target flows', () => {
  ...
  it('opens knowledge content in a dedicated reader instead of the detail drawer', () => {
    const knowledge = readSource('src/domains/knowledge/pages/KnowledgePage.vue')
    const reader = readSource('src/domains/knowledge/pages/KnowledgeReaderPage.vue')
    const router = readSource('src/router/index.ts')

    expect(knowledge).toContain("name: 'KnowledgeReader'")
    expect(knowledge).not.toContain('@click="openDetail(record)"')
    expect(reader).toContain('<FilePreviewRouter')
    expect(reader).toContain('renderSafeMarkdown(')
    expect(router).toContain("name: 'KnowledgeReader'")
    expect(router).toContain("path: '/knowledge/:id([A-Za-z0-9_-]*[0-9_-][A-Za-z0-9_-]*)/view'")
  })

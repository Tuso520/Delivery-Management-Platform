...
describe('standard and knowledge target flows', () => {
  it('routes the primary standard entry to the unified standard page', () => {
    ...
    expect(router).not.toContain("path: '/knowledge/:id(")
  ...
  it('uses target standard APIs for summary, configured categories, versions and review', () => {
    ...
    expect(source).toContain("<p>{{ selectedCategory?.description || '' }}</p>")
  ...
  it('supports all three knowledge content types with explicit versions', () => {
    ...
    expect(source).toContain('filePreview.openPreview({')
    expect(source).toContain('filePreview.openMarkdownPreview({')
  ...
  it('keeps the standard detail route and removes the knowledge detail drawer route', () => {
    ...
    expect(knowledge).not.toContain('<a-drawer')
    expect(knowledge).not.toContain("name: 'KnowledgeDetail'")
    expect(router).not.toContain("name: 'KnowledgeDetail'")
  ...
  it('uses the dedicated Figma frames for both standard and knowledge libraries', () => {
    ...
    expect(standard).toContain('class="standard-table"')
    expect(standard).toContain('<BusinessTable')
    expect(standard).toContain('min-width: 937px')
    expect(standard).toContain(':width="90"')
    expect(standard).toContain(':width="130"')
    expect(standard).toContain(':width="170"')
    expect(standard).toContain(':width="182"')
    expect(standard).toContain('height: 44px')
    expect(standard).not.toContain('<PageToolbar')
    expect(standard).not.toContain('<table')
    ...
    expect(knowledge).toContain('class="knowledge-table"')
    ...
    expect(knowledge).toContain('min-width: 937px')
    ...
    expect(knowledge).toContain('@click="openKnowledgeMaterial(record)"')
    expect(knowledge).toContain('{{ knowledgeMaterialName(record) }}')
    expect(knowledge).not.toContain('<PageToolbar')
    expect(knowledge).not.toContain('<table')
  })

  it('opens knowledge content in the unified preview modal without reader routes', () => {
    const knowledge = readSource('src/domains/knowledge/pages/KnowledgePage.vue')
    const router = readSource('src/router/index.ts')
    const app = readSource('src/App.vue')

    expect(knowledge).toContain('filePreview.openPreview({')
    expect(knowledge).toContain('filePreview.openMarkdownPreview({')
    expect(knowledge).toContain('openKnowledgeMaterial(created)')
    expect(knowledge).not.toContain("name: 'KnowledgeReader'")
    expect(knowledge).not.toContain('@click="openDetail(record)"')
    expect(router).not.toContain("name: 'KnowledgeReader'")
    expect(app).toContain('<AttachmentPreviewModal')
  })

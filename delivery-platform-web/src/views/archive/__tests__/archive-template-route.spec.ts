...
describe('archive template route state', () => {
  it('uses the project-overview frame while preserving the archive workspace', () => {
    ...
    expect(page).toContain('grid-template-columns: 270px minmax(0, 1fr)')
    expect(page).toContain(':scroll="{ minWidth: 760 }"')
    expect(page).toContain(':title="resolveProjectArchiveFileName(record)"')
    expect(page).toContain('v-if="canDownloadItem(record) && record.file"')
    ...
    expect(page).not.toContain("slotName: 'version'")
    expect(page).not.toContain("t('archive.updateFile')")
    expect(page).not.toContain('openUpload(record)')
    expect(page).toContain('v-if="canDownloadItem(record) && record.file"')
    expect(page).toContain('v-if="record.canDeleteFile && record.file"')

...
describe('application shell layout', () => {
  ...
  it('renders route-derived menu expansion and an explicit empty state', () => {
    ...
    expect(sidebarSource).toContain("'--menu-icon-url'")
  ...
  it('centers menu icons, labels and chevrons with flex containers', () => {
    ...
    expect(sidebarSource).toMatch(/\.figma-menu-icon\s*\{[^}]*display:\s*block[^}]*background-color:\s*currentColor[^}]*mask-image:\s*var\(--menu-icon-url\)/s)

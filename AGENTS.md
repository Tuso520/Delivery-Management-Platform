# AGENTS.md - 交付管理平台工程协作规范

本文是交付管理平台的工程协作入口。所有 AI 或人工改动都必须以源码、`docs/` 文档和本文件为准，不再维护历史阶段目录。

## 项目身份

- 名称：交付管理平台，Delivery Management Platform。
- 定位：面向软件交付中心、项目经理、专业工程师、采购、财务、HSE 与标准管理员的企业级交付管理系统。
- 范围：项目台账、项目档案、文件审批、交付流程、检查模板、文档模板、知识库、绩效、组织权限和系统配置。
- 部署：本地使用 Docker Compose 或模拟服务测试，生产使用 Git 拉取部署。

## 技术栈

```text
Frontend: Vue 3 + TypeScript + Vite + Pinia + Vue Router + Arco Design Vue
Backend: NestJS 11 + TypeScript + Prisma 5 + MySQL 8
Cache: Redis 7
Storage: MinIO, S3-compatible
Runtime: Docker / Docker Compose + Nginx + Node.js 20 + pnpm
```
...
10. 文档只维护在 `README.md`、`DEPLOYMENT.md`、`CONTRIBUTING.md`、`SECURITY.md` 和 `docs/`。
...
- 主分支：`main`。
- 提交格式：Conventional Commits，例如 `docs(project): standardize repository layout`。
- 推送目标：`origin/main`。
...
6. 测试验收：执行单元、构建、模拟服务、Docker 配置和关键页面验证。

...
const knowledgeCatalogPath = join(
  projectRoot,
...
let articles = [
  ...
  },
  {
    ...
    fileExt: null,
    markdownContent:
      "现场调试前应完成图纸版本核对、设备铭牌确认、回路检查和安全隔离记录。",
    sourceStatus: "Ready",
    needsRevision: false,
    background: null,
    standardPractice: null,
...
const localUsers = [
  ...
  },
  {
    ...
    username: "elec_chen",
    realName: "陈电气",
    email: "elec.chen@delivery-platform.local",
  },
  {
    id: "user-sw-zhao",
    username: "sw_zhao",
    realName: "赵软件",
    email: "sw.zhao@delivery-platform.local",
  },
];
...
const archiveUploadGuides = {
  "01_presale": [
    [
      "合同与中标文件",
      "上传合同、补充协议、中标通知书和商务确认记录，便于后续验收与回款核对。",
    ],
    [
      "项目启动资料",
      "上传启动会纪要、项目章程、范围边界说明和客户联系人清单。",
    ],
    [
      "需求确认材料",
      "上传需求调研表、客户确认邮件、需求冻结记录和变更风险说明。",
    ],
    [
      "项目成员任命",
      "上传项目经理、软件、电气、采购、财务等岗位任命与职责分工。",
    ],
  ...
  "02_design": [
    ...
    [
      "方案评审记录",
      "上传方案评审会议纪要、评审问题清单、整改闭环和客户确认。",
    ],
    ["电气设计文件", "上传控制柜图纸、原理图、接线图、IO 分配和电源设计说明。"],
    [
      "软件设计文件",
      "上传架构说明、部署拓扑、接口清单、账号权限和数据点配置。",
    ],
  ...
  catalog.modules.forEach((module, moduleIndex) => {
    ...
    module.contents.forEach((content) => {
      ...
      const article = {
        ...
        standardPractice:
          "按知识分类维护文件索引，附件必须可下载、可在线查阅，并按修订状态维护。",
        steps: null,
        notes: content.updateFrequency
          ? `更新频率：${content.updateFrequency}`
          : null,
        commonMistakes: null,
        relatedFlow: "workflow-delivery",
        relatedChecklist: "checklist-delivery",
...
function buildCategoryTree(rows) {
  const clones = new Map(
    rows.map((category) => [category.id, { ...category, children: [] }]),
  );
  const roots = [];
  rows.forEach((category) => {
    const node = clones.get(category.id);
    const parent = category.parentId
      ? clones.get(category.parentId)
      : undefined;
    if (parent) {
...
function buildFileRevisionDiff(revisionId) {
  const revision = attachments.find(
    (item) =>
      item.id === revisionId &&
...
function applyApprovalDecision(task, decision, comment) {
  ...
  if (decision === "Rejected") {
    ...
    revision.remark = JSON.stringify({
      ...payload,
      decision,
      comment,
      decidedAt: now(),
  ...
  revision.remark = JSON.stringify({
    ...
    decision,
    comment,
    originalAttachmentId: originalId,
    decidedAt: now(),
  });
  const article = articles.find((item) => item.id === articleId);
  if (article) {
    const currentVersion = Number.parseFloat(
      String(article.version || "V1.0").replace("V", ""),
    );
...
function buildLocalProjects() {
  ...
  return seed.map((row, index) => {
    ...
    return {
      ...
      electricalOwnerId: elec.id,
      softwareOwnerId: sw.id,
      currentStage,
      projectStatus,
      riskLevel,
...
function buildLocalArchiveItems(projects) {
  return projects.flatMap((project) =>
    archiveStageDefinitions.flatMap((stage, stageIndex) =>
      archiveUploadGuides[stage.stageCode].map(
        ([name, usageDescription], itemIndex) => {
...
function attachArchiveRelations(item) {
  ...
  return {
    ...
    responsibleUser: {
      ...
      realName: responsibleUser.realName,
      username: responsibleUser.username,
    },
    reviewUser: {
      id: reviewUser.id,
      realName: reviewUser.realName,
...
function archiveStatisticsForProject(projectId) {
  const items = archiveItems.filter(
  ...
  return {
    ...
    stages: archiveStageDefinitions.map((stage) => {
      const stageItems = items.filter(
        (item) => item.stageCode === stage.stageCode,
      );
      const stageCompleted = stageItems.filter((item) =>
        ["Approved", "Archived"].includes(item.status),
      ).length;
      return {
        stageCode: stage.stageCode,
        ...
        completionRate: stageItems.length
          ? Math.round((stageCompleted / stageItems.length) * 100)
          : 0,
      };
    }),
  };
...
function decideProjectFile(fileId, decision, comment) {
  const file = projectFiles.find(
    (entry) => entry.id === fileId && !entry.deletedAt,
  );
  if (!file) return null;
  const review = fileReviews.find(
    (entry) => entry.fileId === fileId && entry.reviewStatus === "Pending",
  );
  if (review) {
  ...
  if (decision === "Approved") {
    ...
    file.updatedAt = now();
    if (item) {
      const hasApproved = projectFiles.some(
        (entry) =>
...
function parseMultipart(buffer, contentTypeHeader) {
  ...
  while (cursor >= 0) {
    ...
    if (headerEnd > 0) {
      ...
      const disposition =
        headerText.match(/content-disposition:\s*([^\r\n]+)/i)?.[1] ?? "";
...
function samplePdf(item = {}) {
  const safeName = (item.originalName || "knowledge-preview.pdf").replace(
    /[^\x20-\x7e]/gu,
    " ",
  );
  const stream = [
...
function localToolItems() {
  return [
    ...
    {
      id: "tool-quality-complete",
      categoryId: "tool-cat-quality",
      name: "资料完整率检查",
    ...
    {
      id: "tool-finance-currency",
      categoryId: "tool-cat-finance",
      name: "多币种金额换算",
    ...
    {
      id: "tool-review-logistics",
      categoryId: "tool-cat-review",
      name: "进出口资料清单",
...
async function handleApi(req, res, url) {
    ...
    sendJson(
      ...
      envelope({
        ...
        recentProjects: [
          {
            id: "project-1",
            projectCode: "CN-SW-2026-001",
            projectName: "软件交付样板项目",
            countryCode: "CN",
            status: "Active",
            riskLevel: "Medium",
            createdAt: now(),
          ...
          {
            id: "project-2",
            projectCode: "VN-EL-2026-001",
            projectName: "电气调试样板项目",
            countryCode: "VN",
            status: "Delayed",
            riskLevel: "High",
            createdAt: now(),
  ...
  if (req.method === "PUT" && categoryMatch) {
    ...
    const nextName =
      body.name == null ? category.name : String(body.name).trim();
    if (!nextName) {
      sendJson(res, envelope(null, "分类名称不能为空", 400), 400);
      return;
    }
    ...
    if (body.sortOrder !== undefined)
      category.sortOrder = Number(body.sortOrder) || category.sortOrder;
  ...
  if (req.method === "GET" && path === "/knowledge/articles") {
    ...
    if (categoryId) {
      const categoryIds = new Set([
        categoryId,
        ...descendantCategoryIds(categoryId),
      ]);
    ...
    if (keyword) {
      list = list.filter(
        (item) =>
          ...
          item.markdownContent?.includes(keyword),
      );
    }
    if (status) list = list.filter((item) => item.status === status);
    if (sourceStatus)
      list = list.filter((item) => item.sourceStatus === sourceStatus);
    sendJson(
      res,
      envelope(
        page(
          withArticleFileCounts(list),
  ...
  if (req.method === "GET" && articleMatch) {
    ...
    sendJson(
      res,
      envelope(article ? withArticleFileCounts([article])[0] : null),
      article ? 200 : 404,
    );
    return;
  }
  ...
  if (req.method === "POST" && path === "/knowledge/articles") {
    const body = await readJson(req);
    const category =
      categories.find((item) => item.id === body.categoryId) ?? categories[0];
    const article = {
      ...articles[0],
      id: `article-${Date.now()}`,
  ...
  const deprecateMatch = path.match(
    /^\/knowledge\/articles\/([^/]+)\/deprecate$/,
  );
  ...
  const revisionSubmitMatch = path.match(
    /^\/knowledge\/articles\/([^/]+)\/files\/([^/]+)\/revisions$/,
  );
  if (req.method === "POST" && revisionSubmitMatch) {
    ...
    const pendingRevision = attachments.find(
      (item) =>
        item.ownerType === "KnowledgeFileRevision" &&
    ...
    const filePart = parts.find((part) => part.filename) ?? {
      ...
      data: Buffer.from(
        `Local revision content for ${original.originalName}`,
        "utf8",
      ),
    };
    const name = filePart.filename || `更新-${original.originalName}`;
    const ext =
      extname(name).replace(".", "").toLowerCase() ||
      fileExt(original.originalName);
    ...
    const storagePath = join(
      targetDir,
      `${Date.now()}-${Math.random().toString(16).slice(2)}-${safeFileName(name)}`,
    );
  ...
  const revisionDiffMatch = path.match(
    /^\/knowledge\/file-revisions\/([^/]+)\/diff$/,
  );
  ...
  if (req.method === "DELETE" && attachmentMatch) {
    const index = attachments.findIndex(
      (attachment) => attachment.id === attachmentMatch[1],
    );
    if (index < 0) {
  ...
  const contentMatch = path.match(
    /^\/attachments\/([^/]+)\/(?:content|preview-content)$/,
  );
  if (req.method === "GET" && contentMatch) {
    const item = attachments.find(
      (attachment) => attachment.id === contentMatch[1],
    );
    if (!item) {
      sendJson(res, envelope(null, "附件不存在", 404), 404);
      return;
    ...
    sendRaw(
      res,
      Buffer.from(`Local preview content for ${item.originalName}`, "utf8"),
  ...
  if (req.method === "GET" && dictMatch) {
    sendJson(
      res,
      envelope(
        dictionaries[dictMatch[1]] ?? {
          id: dictMatch[1],
          categoryCode: dictMatch[1],
          categoryName: dictMatch[1],
          items: [],
        },
      ),
    );
          ...
          [
            {
              ...
              categoryId: "wf-cat-1",
              name: "软件部署交付流程",
              documentNo: "WF-SW-001",
              version: "V1.0",
              status: "Active",
              responsibleRole: "SOFTWARE_ENGINEER",
              applicableScope: "{}",
              triggerCondition: "{}",
              inputMaterials: "{}",
              steps: "{}",
          ...
          [
            {
              ...
              templateCode: "CK-SW-001",
              templateName: "软件交付检查模板",
              countryCode: "CN",
              projectType: "software",
              stageCode: "delivery",
              version: "V1.0",
  ...
  if (req.method === "GET" && path === "/templates") {
    const templates = [
      {
        ...
        applicableRole: "PROJECT_MANAGER",
        language: "zh-CN",
        fileFormat: "docx",
        storagePath: null,
        status: "Published",
    ...
    const templatesWithFiles = templates.map((template) => {
      const attachment = attachments.find(
        (item) => item.fileExt === template.fileFormat,
      );
      return {
        ...template,
  ...
  if (req.method === "POST" && path === "/projects") {
    ...
    const project = {
      ...
      members: [
        ...
        makeProjectMember(
          id,
          findLocalUser("user-elec-chen"),
          "ELEC_LEADER",
          3,
        ),
        makeProjectMember(
          id,
          findLocalUser("user-sw-zhao"),
          "SOFTWARE_LEADER",
          4,
        ),
      ],
  ...
  if (req.method === "GET" && projectDetailMatch) {
    const project = localProjects.find(
      (item) => item.id === projectDetailMatch[1] && !item.deletedAt,
    );
  ...
  if (req.method === "DELETE" && projectDeleteMatch) {
    const project = localProjects.find(
      (item) => item.id === projectDeleteMatch[1] && !item.deletedAt,
    );
  ...
  const archiveGenerateMatch = path.match(
    /^\/projects\/([^/]+)\/archives\/generate$/,
  );
  if (req.method === "POST" && archiveGenerateMatch) {
    const project = localProjects.find(
      (item) => item.id === archiveGenerateMatch[1] && !item.deletedAt,
    );
    if (!project) {
      sendJson(res, envelope(null, "项目不存在", 404), 404);
      return;
    }
    const existing = archiveItems.some(
    ...
    sendJson(
      ...
      envelope({
        message: "档案目录生成成功",
        totalItems: 70,
        templateName: "标准交付档案模板",
      }),
    );
    return;
  }

  const archiveStatsMatch = path.match(
    /^\/projects\/([^/]+)\/archives\/statistics$/,
  );
  ...
  const archiveItemMatch = path.match(
    /^\/projects\/([^/]+)\/archives\/([^/]+)$/,
  );
  if (req.method === "GET" && archiveItemMatch) {
    ...
    sendJson(
      res,
      envelope(item ? attachArchiveRelations(item) : null),
      item ? 200 : 404,
    );
    return;
  }
  ...
  const archiveNotApplicableMatch = path.match(
    /^\/projects\/([^/]+)\/archives\/([^/]+)\/mark-not-applicable$/,
  );
          ...
          [
            {
              ...
              content: "完成部署联调和资料归档检查。",
              workHours: 6.5,
              projectProgress: "交付阶段推进中",
              paymentProgress: "本周无新增回款",
              riskNotes: "暂无重大风险",
              nextPlan: "继续完成验收资料",
              status: "Draft",
              project: {
  ...
  if (req.method === "GET" && path === "/archive-templates") {
    const templates = [
      ...
      [
        "archive-template-oversea",
        "AR-OS-001",
        "海外项目档案模板",
        null,
        "补充跨文化沟通、清关、物流和外币回款资料。",
      ],
      [
        "archive-template-china",
        "AR-CN-001",
        "国内项目档案模板",
        null,
        "适用于国内客户现场交付与资料归档。",
      ...
      envelope([
        {
          ...
          status: "Active",
          sortOrder: 1,
          manager: { id: "user-admin", realName: "系统管理员" },
          userCount: 2,
          children: [],
        },
      ]),
    ...
    sendJson(
      ...
      envelope(
        page(
          [
            {
              id: "approval-project-archive",
              templateCode: "PROJECT_ARCHIVE_REVIEW",
              templateName: "项目档案审核",
              ...
              steps: [
                {
                  ...
                  requiredCount: 1,
                  approverType: "role",
                  approverValues: ["PROJECT_MANAGER"],
                },
              ],
            },
            {
              id: "approval-knowledge",
              templateCode: "KNOWLEDGE_REVIEW",
              templateName: "知识发布审批",
              ...
              steps: [
                {
                  id: "step-knowledge-file-1",
                  stepOrder: 1,
                  stepName: "管理员审核",
                  mode: "SINGLE",
                  requiredCount: 1,
                  approverType: "user",
                  approverValues: ["user-admin"],
                },
              ],
            },
          ],
    ...
    sendJson(
      ...
      envelope(
        projectFiles
          .filter(
            (file) =>
              file.archiveItemId === archiveFilesMatch[1] && !file.deletedAt,
          )
          .map(fileResponse),
      ),
    );
  ...
  if (req.method === "POST" && path === "/files/upload") {
    ...
    const name = safeFileName(
      filePart.filename || `项目档案文件-${Date.now()}.dat`,
    );
    const targetDir = join(
      localStorageRoot,
      "files",
      fields.projectId,
      fields.archiveItemId || "general",
    );
    await mkdir(targetDir, { recursive: true });
  ...
  if (req.method === "GET" && filePreviewMatch) {
    const file = projectFiles.find(
      (entry) => entry.id === filePreviewMatch[1] && !entry.deletedAt,
    );
  ...
  const filePreviewSessionMatch = path.match(
    /^\/files\/([^/]+)\/preview-session$/,
  );
  if (req.method === "GET" && filePreviewSessionMatch) {
    const file = projectFiles.find(
      (entry) => entry.id === filePreviewSessionMatch[1] && !entry.deletedAt,
    );
    if (!file) {
  ...
  const fileDownloadMatch = path.match(
    /^\/files\/([^/]+)\/(?:download|preview-content|signed-content)$/,
  );
  if (req.method === "GET" && fileDownloadMatch) {
    const file = projectFiles.find(
      (entry) => entry.id === fileDownloadMatch[1] && !entry.deletedAt,
    );
    if (!file) {
      sendJson(res, envelope(null, "文件不存在", 404), 404);
      return;
    ...
    sendRaw(
      res,
      Buffer.from(
        `Local project archive file content for ${file.originalName}`,
        "utf8",
      ),
  ...
  if (req.method === "GET" && fileDetailMatch) {
    const file = projectFiles.find(
      (entry) => entry.id === fileDetailMatch[1] && !entry.deletedAt,
    );
    sendJson(res, envelope(file ? fileResponse(file) : null), file ? 200 : 404);
    return;
  }
  ...
  if (req.method === "DELETE" && fileDetailMatch) {
    ...
    if (file) {
      file.deletedAt = now();
      const item = archiveItems.find(
        (entry) => entry.id === file.archiveItemId,
      );
      if (item) {
        const hasFiles = projectFiles.some(
          (entry) =>
            entry.archiveItemId === item.id &&
            !entry.deletedAt &&
            entry.id !== file.id,
              ...
              assessments: [
                {
                  ...
                  finalScore: 87,
                  period: "2026-Q3",
                  evidenceNote: "完成本地部署测试",
                  user: { id: "user-sw", realName: "软件负责人" },
                  assessor: { id: "user-admin", realName: "系统管理员" },
                },
              ],
            },
          ],
          ...
          [
            {
              id: "retro-1",
              projectId: "project-1",
              summary: "部署联调过程需要提前固化检查清单。",
              lessonsLearned: "环境变量、版本包和回滚脚本需同步归档。",
              problemCategory: "process",
              status: "InProgress",
              project: {
                id: "project-1",
                projectCode: "CN-SW-2026-001",
              ...
              actions: [
                {
                  ...
                  status: "Open",
                  dueDate: "2026-07-10",
                  owner: { id: "user-sw", realName: "软件负责人" },
                },
              ],
            },
          ],
  ...
  if (req.method === "GET" && path === "/system-operations/storage") {
    ...
    sendJson(
      res,
      envelope({
        bucket: "local-delivery-test",
        available: true,
        provider: "local",
        attachmentCount: attachments.length,
        attachmentBytes: String(attachmentBytes),
        backupCount: 1,
      }),
    );
    return;
          ...
          [
            {
              ...
              backupType: "database",
              status: "Success",
              storagePath: "local://backup-1.zip",
              fileSize: "102400",
              createdAt: now(),
              requester: {
                id: "user-admin",
  ...
  const systemConfigList = [
    ...
    {
      id: "cfg-report",
      configKey: "report.reminder_hour",
      configValue: "18",
      description: "日报提醒小时",
      configType: "number",
      updatedBy: "user-admin",
  ...
  if (req.method === "GET" && path === "/system-config/batch") {
    ...
    const data = Object.fromEntries(
      keys.map((key) => [
        key,
        systemConfigList.find((item) => item.configKey === key)?.configValue ??
          null,
      ]),
    );
  ...
  if (
    req.method === "GET" &&
    ["/okr/objectives", "/okr/my-objectives", "/okr/team-objectives"].includes(
      path,
    )
  ) {
    sendJson(res, envelope([]));
  ...
  if (req.method === "GET" && path === "/tools/items") {
    ...
    const items = localToolItems().filter(
...
async function serveStatic(req, res, url) {
  ...
  const filePath =
    safePath.startsWith(webDist) && existsSync(safePath)
      ? safePath

...
test('missing immutable images use a checksummed and resumable SSH preload before data reset', () => {
  ...
  for (const contract of [
    ...
    'docker image tag "$backend_tag" "delivery-platform-relay/backend:$3"',
    'docker image tag "$migration_tag" "delivery-platform-relay/migrator:$3"',
    ...
    'control_files+=(release/image-bundle.map.json)',
    'case "$REUSE_IMAGE_CONTENT"',

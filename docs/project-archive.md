...
9. “整体完成度”固定按“完成文件夹数 ÷ 目录规模”计算；同一文件夹内增加多个文件不会扩大完成度分母。文件夹至少有一个文件且其中所有文件均为完成状态时计为完成文件夹。
10. 单文件上传遇到 502、503、504、连接中断或客户端超时时，前端使用同一个 `Idempotency-Key` 做有限退避重试，并为每次尝试重新创建 multipart 请求；MinIO 客户端同时执行有限短退避。业务校验错误不重试，多文件仍按队列逐个提交。
...
- 测试环境运行时验收会在真实 NestJS、MySQL、Redis 和 MinIO 上连续上传两个独立项目档案文件，核对目录文件列表与计数后软归档测试数据。

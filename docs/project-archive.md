...
3. 文件夹按快照 `sortOrder/name` 排序，目录数量显示有效档案项数量；历史标准目录下没有任何文件关联的“相关交付文件”占位项不在文件表显示且不计数，有真实文件的历史项仍保留，避免误报和数据丢失。空占位项只作为兼容上传目标保留，继续支持其既有多文件策略。
...
6. 顶部“上传”默认选择当前目录中优先未上传的可上传档案项；“档案项”使用 Arco Select，候选项只来自所选项目绑定的版本化档案模板快照，因此切换项目或模板会同步改变目录与档案项。候选项按“目录 / 具体档案项”展示；档案项名称为通用“项目交付文件”“相关交付文件”或与目录同名时只显示目录，避免冗余。Arco Upload 支持一次选择多个文件，但不渲染内置文件列表，因此不存在单文件“开始/重试”动作；选中文件由平台列表统一管理并由弹窗底部“上传”顺序提交。多个独立文件仅允许提交到启用 `allowMultipleFiles` 的档案项，后端继续做最终校验。

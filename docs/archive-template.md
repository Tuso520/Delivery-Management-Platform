...
| 状态列           | 不存在                                           | 存在                    | 从正常列表删除                     |
| 目录规模         | 原节点为两级数量                                 | `45 文件夹 / 69 文件项` | 按当前业务语义仅展示文件夹数       |
...
| 目录规模     |      111px |        111px | 居中     | 居中     | 文件夹数                                   | 单行                   |
...
- 查看：打开 Arco Modal 并读取模板、版本和目录结构接口。弹窗固定为协调的工作台宽度，顶部集中版本、保存和发布操作，正文使用 Arco Card 紧凑展示文件夹清单。
- 创建：创建模板及初始草稿版本。
- 创建新版本：调用版本接口并打开新草稿。
- 停用：后端权限校验并保留历史发布版本和项目快照。
- 模板只定义文件夹名称、说明和顺序，不定义“文件项”、文件数量、扩展名、命名规则或单文件/多文件策略。空文件夹不再渲染“该文件夹暂无文件项”。
...
`ArchiveTemplateFolder` 保存模板文件夹、稳定键和排序；`ArchiveTemplateVersionItem` 仅保留为历史发布版本兼容结构，新版模板编辑与发布不再要求或创建文件项。
项目创建时复制发布版本为项目快照，并保存模板及版本主键，不通过显示名称关联。
本次列表重构不改变 Prisma schema，不新增 migration。

## 文件存储与版本联动

- 模板只定义目录结构，不在模板中保存文件实体或文件数量限制。

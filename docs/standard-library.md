...
拒绝归档。历史无版本标准归档时不会构造空版本审核查询，仍按同一事务软归档主记录并
写入操作日志。文件下载通过统一文件权限和审计链路，数据库只保存文件索引和版本元数据。

测试环境旧生成器遗留的 `TS-* / 随机测试标准 *` 草稿由部署迁移执行
dry-run、apply 和只读 verify 后软归档；清理器只在 `DEPLOY_ENV=test` 运行，要求无当前
发布版本和无待审任务，不物理删除标准或版本。通用测试数据生成器不再创建随机标准，
避免再次产生不符合文件版本约束的记录。

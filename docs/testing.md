...
源码静态事实由 `node scripts/verify-doc-facts.mjs` 在每次验收中重新计算。当前仓库扫描范围为 725 个受版本控制或待纳入版本控制的文件；前端 202 个 TypeScript/Vue 文件、25 个 `views/` Vue 文件、27 个运行时 API 文件和 48 个测试文件；后端 249 个 TypeScript 文件、27 个 Controller、42 个 Service、29 个 Module、176 个 HTTP 路由和 49 个 Prisma migration。以上数字只作为本次交付快照，后续发布仍以脚本实时计算结果为准。
...
这些结果只证明仓库侧轻量质量与视觉门禁通过。真实 MySQL、Redis、MinIO、47 个 migration、三个 migrator、API E2E 和浏览器权限矩阵，必须由同一 Release 的 GitHub integration 产生 PASS 后才可部署测试服务器；生产还必须取得测试验证凭据和 Environment 审批。
...
3. integration 必须使用上述正式发布物启动真实 MySQL、Redis、MinIO、API 和两个 Worker，执行 47 个 migration、三个数据 migrator、二次幂等 seed、真实 API/权限/浏览器 E2E。

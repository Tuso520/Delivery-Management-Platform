...
本文以前端正式架构为业务入口，记录交付管理平台后端当前的**业务域、数据模型、接口、权限、统一审核、统一文件、集成和迁移边界**。
...
3. 角色作为权限集合；业务 Controller 使用权限码，Service 执行数据范围和业务条件终检。
4. 项目、档案、审核、标准、知识统一使用清晰的版本和状态模型。
5. 项目文件、标准文件和知识文件统一接入文件中心和审核中心。
6. 运行时只暴露当前前端与集成方实际使用的业务接口。
7. 所有高风险业务动作使用专用命令接口，通用更新接口不接收受控状态字段。
8. 迁移源表只读保留，应用运行时不双读或双写迁移源。
...
角色权限管理面是唯一例外：角色目录、角色增删改和权限集合替换必须同时满足 `SUPER_ADMIN` 角色约束，防止普通管理角色通过修改自身权限完成提权。`SUPER_ADMIN` 为受保护角色，固定启用并在运行时绕过普通权限集合；系统拒绝删除、取消其权限或移除/禁用最后一个可用超级管理员账号。
数据范围 = 可访问数据集合
业务状态 = 当前是否允许执行
```
...
运行时 StandardVersion 必须引用有效、未归档且 checksum 已校验的 MinIO FileVersion；`structuredContent`、`applicability` 仅在渐进迁移阶段作为只读来源存在，不得进入 DTO、公开响应或新写入。
...
3. 运行 `prisma:audit-archive-migration`，审查档案层级和对象存储引用；任何 ERROR 都必须在数据 apply 前阻断。`STORED_LEVEL_MISMATCH` 可由父子关系确定性重建层级，`FOLDER_WITH_DIRECT_FILES` 可确定性拆分为目录和同名合成档案项，这两个 REVIEW 保留在预审报告但不阻断；任何其他或未来新增的 REVIEW 必须默认阻断。
4. 对 `prisma:migrate-target-content --strict` 做 apply 前门禁，再使用 `--apply --actor-user-id=<active-id>` 或 `--actor-username=<active-username>`，最后以 `--verify --strict` 校验标准/知识内容和版本覆盖率；strict dry-run 的任何 ERROR 都必须在首次写入前阻断。
5. 对 `prisma:migrate-target-foundation --strict` 做 apply 前门禁；确定性目录/档案项拆分只计入迁移计划，清理所有不可自动判断的 ERROR/REVIEW 后执行 `--apply --strict --actor-user-id=<active-id>` 或 `--actor-username=<active-username>`，再以 `--verify --strict` 校验数量、外键和关系。apply 期间若仍产生 finding，只能写失败审计并中止。
...
默认 Compose 的 `backend-migrate` 按上述依赖顺序自动执行档案 ERROR 预审和三个 migrator 的 dry-run/apply/只读 verify，并以同一启用审计账号记录写入；foundation apply 只有在 strict dry-run 无阻断 finding 后才能开始，任一步失败都阻断 API 和 Worker。数据库 UI 翻译表通过迁移归档保留原数据，但 Prisma 模型、seed 和运行时读写已移除，不能作为长期兼容层继续使用。

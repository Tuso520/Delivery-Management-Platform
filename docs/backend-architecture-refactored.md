...
6. 配置独立 32 字节 Base64 `INTEGRATION_SECRET_ENCRYPTION_KEY`，先 dry-run `prisma:migrate-integration-secrets`，再执行 `--apply --actor-user-id=<active-id>` 或 `--actor-username=<active-username>`；确认明文 Secret 行为 0。
...
- 代码回滚前先停止 Worker，避免旧代码消费新 Outbox 或处理任务。

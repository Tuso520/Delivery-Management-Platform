...
完整路由、页面、Query Key、组件和运行边界见 [前端页面架构](frontend-architecture.md)，端到端状态流见 [前端业务流程](frontend-business-flows.md)，正式设计约束见 [前端重构实施规范](frontend-architecture-refactored.md)。
...
完整模型、接口、并发和迁移契约见 [后端重构实施规范](backend-architecture-refactored.md)。

## 核心业务模型

### 项目与档案

- `Project` 只使用生命周期 `DRAFT / ACTIVE / PAUSED / COMPLETED / CANCELLED` 和固定交付阶段；普通字段、阶段、验收和状态使用不同命令。
...
目标运行时已切换到统一项目状态、档案、文件、审核、标准、知识与集成模型。旧流程、检查模版、文档模版、知识文章、附件、报告、绩效等表不再由应用读写，只作为未破坏的历史迁移源保留。生产升级必须先备份，按 dry-run → apply → verify 执行数据脚本；详细顺序和回滚边界见 [部署运维](deployment.md)。

...
- GitHub 质量检查通过后只构建一次前端静态包；后端 Git 树内容 SHA-256 已存在时直接复用 GHCR 的 runtime、migrator，不存在时才构建。`release-manifest.json` 绑定完整 Git SHA、镜像 digest、前端 checksum 与 migration 数量。migrator 继承精简 runtime 以复用镜像层，使用独立构建缓存且只增加固定版本迁移命令，不携带 builder 工具链；Release ID 只在容器启动时注入，不写入镜像配置，首次内容构建固定 `SOURCE_DATE_EPOCH=0`，因此后端内容相同时跨 Release 复用完全相同的不可变 digest。

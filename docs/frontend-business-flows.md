...
左侧四个分组承担日常业务和系统设置；项目创建/详情/编辑通过可分享深链打开同一个项目列表页上的弹窗或抽屉。
...
2. 用户可切换中文或英文，输入用户名、密码；密码输入框右侧提供显示/隐藏按钮，默认以掩码显示。
...
2. 五张统计卡按 Figma 顺序只读展示项目金额、确收金额、项目总数、进行中项目和今年验收项目。
3. 关键词搜索项目名称、项目编号或客户名称。
4. 已提交的 `keyword` 与项目经理 `sort` 写入 URL query；输入过程不会提前请求，`page`、`pageSize` 作为每批 20 条滚动续载的内部请求状态。
5. 列表按 Figma 固定顺序展示项目名称、项目经理、项目地点、当前阶段、进度、签约/验收时间、合同币种、合同金额、折算人民币、客户类型、合同类型和销售。
6. 金额字段是否返回、项目是否在列表中由后端敏感字段权限和数据范围决定；合同金额与折算人民币统一保存并显示两位小数，前端不自行扩展权限范围。
7. 视图选择器提供“我的项目”“全部项目”“归档项目”；页面默认使用 `scope=mine`，切换范围只改变查询意图，实际结果仍受后端数据范围约束。
8. 列表不设置操作列；查看、编辑、进度和归档动作集中在居中详情弹窗。归档项目复用同一范围选择器与表格，恢复或物理删除不在列表中提供操作。
9. 视图选择、关键词查询和创建操作位于表格附着式工具栏；Figma 未定义刷新按钮，统计卡、工具栏和宽表格构成同一紧凑工作区。
...
入口：`/projects/create`，在项目概览上打开 944px 居中详情弹窗的创建模式。

页面同时加载：项目模块字段配置（国家、币种、项目类型、客户类型、合同类型、产品、项目关键词、阶段和状态）、启用语言，以及销售负责人、项目经理、项目成员三类目的化用户引用。字段配置由 `GET /field-options/module/project` 返回，并与服务端写入终检使用同一组数据库字段选项。
...
入口：`/projects/:projectId` 打开 944px 居中详情弹窗，`/projects/:projectId/edit` 打开同一弹窗的编辑模式；两者保留项目概览的筛选、分页和范围 query。
...
归档、恢复和永久删除继续作为受限后端生命周期能力存在，但当前 Figma 不提供归档列表或物理删除入口。后端只允许 `SUPER_ADMIN` 对无文件、无审核、无财务且无既有审计记录的已归档项目执行物理删除，否则返回 409 并记录失败审计。
...
档案模版列表按 Figma `69:305` 使用 1208px 的固定横向滚动宽度和固定表格布局，
列宽依次为 `280/120/111/111/95/160/149/182px`。可用宽度不足时仅在表格容器内
横向滚动；数据刷新、权限按钮变化和容器尺寸变化不触发列宽重新分配。列表只提供模板名
关键字查询，以及模板名称、当前版本的真实服务端排序；节点没有分页器、状态列或刷新按钮。
完整页面、API、字段配置和版本契约见 [archive-template.md](archive-template.md)。
...
- 模版停用不会破坏已经创建的项目。
- 项目快照不会反向写入模版；当前页面和 API 不再允许创建临时档案项。

### 5.3 模版版本边界

项目快照仍通过稳定键保留模板来源和版本。Figma `43:317` 没有模板同步入口，因此项目档案页面不请求差异接口，也不展示同步弹窗；后端保留的受控同步能力不能绕过稳定键、ADD_ONLY 和审计约束。
...
4. 显示完成率、档案项、必填项、目录规模和完成文件夹，并在 270px 目录中列出所有未归档文件夹。
5. 文件更新和删除能力直接使用后端返回的 `canUpload/canDeleteFile`；删除归档逻辑文件，不删除档案项。

### 6.2 上传文件版本

1. 用户在文件项上选择上传。
2. 无当前文件时默认 `REPLACE`；已有当前文件时默认 `NEW_VERSION`。
3. 页面更新现有文件时提交小版本；变更说明可选，版本号由服务端生成。
4. 前端从 `FILE_TYPE` 字段配置读取启用扩展名，并与快照允许类型取交集。
5. 前端发送 multipart、上传进度和 `Idempotency-Key`，超时 120 秒。
6. 服务端再次校验字段配置、快照格式/大小/命名规则、文件签名、权限和项目数据范围；成功后刷新项目档案树。
...
上传仍在 `FileAsset.originalName` 保存完整原始文件名；MinIO 对象键按 UTF-8 字节安全截断到单段上限以内，超长中文名称不会导致对象存储拒绝，也不会改变列表和下载时展示的原名。
...
- 列表默认加载 20 条，关键词和状态写入 URL；接近容器底部时按内部页码续载，达到总数后停止。
...
页面严格按 Figma `70:322` 显示标准总数、累计访问和累计下载，提供关键词查询与“新增”按钮；主工作区以“交付阶段/管理领域”两个 44px 页签和左侧分类驱动列表，不显示旧状态筛选器或分页器。表格只包含“资料标题、当前版本、生效日期、更新人、操作”五列；1440×900 基线列宽为 `365/90/130/170/182px`，宽屏由标题列吸收剩余空间，窄屏通过右侧唯一横向视口滚动。主工作区自适应撑满应用壳剩余高度，左侧分类和右侧表体分别纵向滚动。

标准类型、交付阶段、管理领域、业务类型、状态、启用状态、当前版本、生效日期和使用国家分别来自 `STANDARD_*` 与 `COUNTRY` 字段配置；页面和后端不维护显示名称枚举。配置重命名、排序、默认值或启停后，分类、列表、详情和表单同步，历史停用值继续显示。

列表资料标题使用标准当前可见版本关联的真实 `FileAsset.originalName`，保留文件扩展名并单行截断；点击标题直接进入统一文件预览，不经过标准详情。列表操作统一为“编辑、归档”，下载只保留在统一预览和版本详情的受控入口。
...
1. 新增时填写标准编号、资料标题、配置化类型、交付阶段、管理领域、业务类型、使用国家、生效日期和首版变更说明。
2. 标准正文只允许统一 `FileVersion`；先通过 `/files/drafts` 上传 DRAFT 文件版本，再创建标准，上传使用幂等键。
3. 标准主数据与标准版本分离；已发布标准再修改时创建新草稿版本。
4. `DRAFT/REJECTED` 版本可按 `revision` 编辑；同一标准存在 `DRAFT/IN_REVIEW` 时前端不再创建另一个活动草稿。
5. 提交统一审核后转 `IN_REVIEW`；最终通过发布、设置生效日期并替换当前发布版本指针，驳回后可继续修订。
6. 历史版本保留真实父子关系、发布状态、当前版本标识和文件版本，可在详情中查看或按权限下载。

### 9.3 标准关系与归档

有编辑权限者可建立或删除六种标准到标准关系：配套表单、配套模版、引用、替代、前置条件、后续标准。关系使用标准主键，不通过名称关联；候选排除自身和已归档标准。

启停与发布状态相互独立，停用不改写历史版本。归档使用软归档；审核中的标准不能归档。详细契约见 [标准库](standard-library.md)。
...
页面严格按 Figma `125:624` 显示标准总数、查看次数和下载次数，提供关键词查询与新增按钮。主工作区是 270px 分类栏和 937px 内容栏，不显示旧分类下拉、状态筛选、刷新按钮或分页器。表格只包含“资料标题、当前版本、生效日期、更新人、操作”五列，列宽固定为 `365/90/130/170/182px`。

页面固定向后端请求第一页 100 条用于 625px 设计表体；后台仍真实支持 `page/pageSize/keyword/categoryId/sortBy/sortOrder`，不做前端假分页或假排序。知识分类与说明由 `/field-options/module/knowledge` 返回，分类计数由 `/knowledge/category-counts` 按稳定主键聚合。新增、编辑只提供启用分类；已停用但仍被历史知识引用的分类继续可查询和展示。
...
详细视觉、字段配置和接口契约见 [知识库](knowledge-library.md)。
...
2. 页面按后端工具分类生成顶部统计卡；统计卡固定为单行，可用宽度不足时在分类区域内横向滚动；点击后将 `category` 写入 URL。
...
7. 下方工具卡片使用响应式网格，每批显示 20 条，接近容器底部时继续显示下一批。

工具中心只负责目录和入口，不把任意内部路由自动变成已实现工具页；配置不存在的站内路径会进入 404。

## 12. 七类设置流程

系统设置从左侧第四个分组进入。路由权限使用“查看或管理”任一满足；没有管理权限时页面保持只读或禁用动作。通知和接口集成不进入目标侧栏，只保留授权深层路由。

### 12.1 用户中心

入口 `/settings`；查看权限 `user:view`。

- 查询用户列表、基本资料、账号状态、所属部门和角色关联。
- 按动作权限新增、编辑、启用、停用、重置密码、分配角色和删除用户。
- 页面不展示角色权限矩阵、操作日志、系统参数或其他设置领域。
...
- 币种编码、名称、排序和启停来自字段配置；汇率页只编辑符号/小数位、维护未锁定汇率、同步在线汇率以及锁定/解锁。
...
- 按规则名或事件类型本地筛选。
- 管理者可维护规则名称、事件类型、站内/飞书渠道、接收策略、模版 ID 和启用状态。
...
入口 `/settings/fields`；查看权限为 `field_setting:view`，写入按 `field_setting:edit`、`field_setting:option_create` 和 `field_setting:option_toggle` 拆分。
...
- 领域事件由独立 Outbox Worker 按通知规则投递站内或飞书；每个事件/用户/通道写 `NotificationDelivery` 回执并按幂等键去重，页面不把“已排队”展示为“外部发送成功”。
...
3. 左侧导航、路由标题、浏览器标题和登录页文案立即更新。

主导航、七类设置和核心业务流程的固定文案已接入同构中英文 key；业务数据不自动翻译。仅深链组织权限维护页仍以中文为主。
...
| 档案     | `archive:view`            | 模板同步、上传、审核和归档使用独立权限；行操作使用后端 `can*`       |
...
| 设置     | 各 `*:view` 或 `*:manage` | 各 `*:manage` 或对应 `user:*` 动作权限                              |
...
合同/金额查看、文件下载、审核决定、成员与权限变更、密码重置、汇率变更、系统设置、集成凭据和同步操作都必须由后端写必要审计。审计记录保留为后端安全能力，不在设置菜单提供独立查询页面或 API。
...
| NAV-01    | 分别准备普通用户、设置只读用户、`SUPER_ADMIN`                                   | 登录并检查侧栏、设置分组、深链、无权限路由                              | 只显示可访问菜单；当前一级图标、标题、展开组与二级选中项一致；详情深链保持对应二级菜单；无权限路由回退；超级管理员可见全部当前路由 |

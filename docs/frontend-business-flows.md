...
页面严格按 Figma `70:322` 显示标准总数、累计访问和累计下载，提供关键词查询与新增按钮；主工作区以“交付阶段/管理领域”两个 44px 页签和左侧分类驱动列表，不显示旧状态筛选器或分页器。表格只包含“资料标题、当前版本、生效日期、更新人、操作”五列，列宽为 `365/90/130/170/182px`，服务端按 `page/pageSize` 分页并在固定表体中续载。

...
10. 视图选择、关键词搜索、刷新和创建操作位于表格附着式工具栏；统计卡、工具栏和无限滚动表格构成同一紧凑工作区。
...
页面显示资料总数、待审核、已发布和本月新增，支持关键词、知识分类、内容形式、状态和每批 20 条滚动续载。知识分类由 `/knowledge/categories` 只读加载为树形选项，当前页面不维护分类。
...
入口 `/settings/fields`；管理权限 `field_setting:manage`。
...
| NAV-01    | 分别准备普通用户、设置只读用户、`SUPER_ADMIN`                                   | 登录并检查侧栏、齿轮、深链、无权限路由                                  | 只显示可访问菜单；当前一级图标、标题、展开组与二级选中项一致；详情深链保持对应二级菜单；无权限路由回退；超级管理员可见全部当前路由 |

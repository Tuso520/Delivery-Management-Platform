...
| 项目档案 | `43:317` | `/archive` | `src/domains/archive/pages/ArchiveWorkspacePage.vue` | 5 项指标、项目选择、模板同步、上传、220px 目录和扁平文件表 |
...
- 项目档案只展示文件夹 + 文件项两级结构，不再提供临时档案项入口。
- 档案模板正常列表只保留刷新和创建模板，不增加搜索、筛选或营销式标题区。
- 标准库和知识库使用相同的 4 指标 + 分类/关键词查询骨架，但保持各自业务列和操作。
- 审批规则没有额外统计区或说明头；筛选和创建操作位于同一工具栏。
- 字段配置严格保留 10 个页签：国家、客户类型、合同类型、产品类型、项目关键词、合同币种、项目阶段、项目状态、岗位、项目类型。正常态没有搜索、筛选或刷新按钮。

...
> 当前部署基线已经切换为“前端静态包 + 后端不可变镜像 + 独立数据层 + 宿主 Nginx 原地切换”。完整架构、测试/生产服务器准备、GitHub Environment、首次接管和成对恢复教程见 [发布与服务器架构 v2](deployment-architecture-v2.md)。本文件后续内容记录旧 `deploy-git.sh` 能力，只用于首发前备份、legacy 恢复和迁移期兜底，不再是日常发布入口。
...
测试数据生成覆盖用户、项目、过程记录、付款、知识、检查模板、工具、组织、
通知、日志、系统配置、集成、汇率、报告、OKR、绩效、审批、技能、培训、复盘和备份记录。
标准库只使用基础 seed 创建的文件版本完整、可审核的确定性标准，不再由通用测试数据生成器补齐随机标准。部署迁移在 `DEPLOY_ENV=test` 下会按 `TS-*` 编码、“随机测试标准 ”名称、草稿状态和无当前发布版本四重签名软归档历史测试残留，并写入操作日志；生产环境不执行该清理。生成器使用发布 commit 派生其余随机分布，便于同一发布复现；国家、币种、客户/合同/产品/项目类型、项目关键词和阶段必须从当前启用的字段配置读取，不得重新维护测试专用枚举。它只允许在 `DEPLOY_ENV=test` 且明确确认时运行。
...
`20260727090000` 至 `20260727110000` 七个迁移依次扩展字段配置契约、拆分字段配置权限、增加项目客户类型、关联知识分类、退役临时档案项权限、导入国家/币种选项，并增加项目多阶段存储及项目经理关系索引。`20260728062000_retire_legacy_project_archive_commands` 清理新版项目档案不再暴露的模板同步与档案项停用权限；`20260728100000_refactor_standard_library_for_figma` 为标准补充交付阶段、管理领域、业务类型、启停和国家稳定关联，先迁移旧分类值，再删除 `Standard.category`、停用 `STANDARD_CATEGORY` 并发布新版标准字段配置；`20260728130000_align_knowledge_library_figma` 将知识分类迁移为 Figma 规定的十个稳定编码，并仅按版本快照迁移确定性的目标知识种子数据。`20260804120000_add_feishu_identity_and_org_permissions` 增加飞书多标识、组织成员关系和分层允许/禁止权限。`20260810090000_add_project_accepted_converted_amount` 增加项目确收折算金额并从既有折算金额回填。源码共 47 个 migration；旧 Compose 与旧 CI 的门禁仍必须保持 47，v2 则只以不可变 Release Manifest 的 `migrations.expectedCount` 为事实源并在部署时注入 Release 专属环境，禁止写入服务器长期 `runtime.env`。应用后必须重复 seed，并执行字段种子幂等、三个 migrator 和运行时一致性只读校验。

...
Actions 的上传和远端部署 SSH/SCP 都启用 30 秒协议级 keepalive，并允许连续 20 次无响应后才中断，避免镜像导出或 TypeScript 构建期间暂时没有终端输出而被中间网络设备误判为空闲连接。该配置不延长部署脚本自身的健康检查或 migration 门禁，也不把失败改为成功。

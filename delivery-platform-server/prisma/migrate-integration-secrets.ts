...
function encryptionKey(): Buffer {
  ...
  if (!configured) throw new Error('INTEGRATION_SECRET_ENCRYPTION_KEY is required with --apply');
  const key = Buffer.from(configured, 'base64');
  if (key.length !== 32) {
    throw new Error('INTEGRATION_SECRET_ENCRYPTION_KEY must be a 32-byte Base64 value');
  }
  return key;
}
...
function decrypt(provider: Provider, value: string, key: Buffer): Record<string, unknown> {
  ...
  if (version !== 'v1' || !ivValue || !tagValue || !encryptedValue) {
    throw new Error('Existing integration ciphertext has an unsupported format');
  }
...
async function main(): Promise<void> {
  ...
  const key = apply ? encryptionKey() : null;
  ...
  for (const record of records) {
    ...
    if (Object.keys(plaintextSecrets).length > 0) report.plaintextSecretRows += 1;
    if (Object.keys(historical).length > 0) report.preservedHistoricalPayloadRows += 1;

    const requiresRewrite =
      record.provider !== provider ||
      Object.keys(plaintextSecrets).length > 0 ||
    ...
    const existingSecrets = record.encryptedConfig
      ? decrypt(provider, record.encryptedConfig, key)
      : {};

...
export class FileController {
  constructor(
    private readonly unifiedFiles: UnifiedFileService,
  ...
  async uploadDraft(
    ...
    try {
      return await this.unifiedFiles.uploadDraftFile(file, dto, actor, idempotencyKey);
    } finally {
      await this.storage.cleanupUnclaimedUpload(file);
    }
  }
...
export class ProjectArchiveFileController {
  ...
  async upload(
    ...
    if (!file) throw new BadRequestException('请选择要上传的文件');
    try {
      return await this.unifiedFiles.uploadProjectArchiveFile(
        projectId,
        itemId,
        file,
        dto,
